...
namespace ChaosIV.WS
{
    public static class WebSocketServer
    {
        ...
        public static WebSocketEvents Events {get;} = new WebSocketEvents();
                    ...
                    if (ServerIsRunning)
                    {
                        if (context.Request.IsWebSocketRequest)
                        {
                            ...
                            try
                            {
                                ...
                                Events.OnClientConnected?.Invoke();
                        ...
                        else
                        {
                            if (context.Request.AcceptTypes.Contains("text/html"))
                            {
                                ...
                                //var HtmlPage = new ArraySegment<byte>(Encoding.UTF8.GetBytes(SimpleHtmlClient.HTML));
                                var HtmlPage = Encoding.UTF8.GetBytes(SimpleHtmlClient.HTML);
                                context.Response.ContentType = "text/html; charset=utf-8";
                                context.Response.StatusCode = 200;
                                context.Response.StatusDescription = "OK";
                                context.Response.ContentLength64 = HtmlPage.Length;
                                await context.Response.OutputStream.WriteAsync(HtmlPage, 0, HtmlPage.Length, CancellationToken.None);
        ...
        private static async Task SocketProcessingLoopAsync(ConnectedClient client)
        {
                ...
                while (socket.State != WebSocketState.Closed && socket.State != WebSocketState.Aborted && !loopToken.IsCancellationRequested)
                {
                    ...
                    if (!loopToken.IsCancellationRequested)
                    {
                        ...
                        if (socket.State == WebSocketState.Open)
                        {
                            ...
                            var bufferCopy = new ArraySegment<byte>(buffer.Array, 0, receiveResult.Count);
                            Events.OnMessage?.Invoke(bufferCopy);

                            //await socket.SendAsync(bufferCopy, receiveResult.MessageType, receiveResult.EndOfMessage, CancellationToken.None);
                        }
                    }
                }
            }
            ...
            finally
            {
                ...
                Events.OnClientDisconnected?.Invoke();
            }
        }
        ...
        public static async Task BroadcastAsync(byte[] message) 
        {
